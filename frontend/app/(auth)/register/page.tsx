...
const ERROR_MESSAGES = {
  ...
  termsNotAccepted: "You must agree to the Terms of Service and Privacy Policy.",
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-indigo-950/10 via-[#08090A] to-purple-950/10 border-r border-[#1D1E22]/60 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="max-w-md space-y-8 z-10">
          <h1 className="text-3xl font-extrabold tracking-tight text-white leading-tight">
            Join businesses already forecasting smarter
          </h1>
...
function RegisterPageContent() {
  ...
  const validateForm = () => {
    if (!fullName || !email || !password || !confirmPassword) {
      setError(ERROR_MESSAGES.requiredFields);
      return false;
    }
    if (password.length < 8) {
    ...
    if (password !== confirmPassword) {
      setError(ERROR_MESSAGES.passwordsDontMatch);
      return false;
    }
    if (!agreeTerms) {
      setError(ERROR_MESSAGES.termsNotAccepted);
      return false;
    }
    return true;
  };
    ...
    try {
      ...
      if (apiError) {
        if (apiError.message.includes("already registered")) {
          setError(ERROR_MESSAGES.emailAlreadyExists);
        } else {
  ...
  return (
    <AuthFormLayout>
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10">
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4 glass-card border-[#1D1E22] p-6 rounded-2xl">
          <FormInput
            label="Full Name"
            id="fullName"
            type="text"
