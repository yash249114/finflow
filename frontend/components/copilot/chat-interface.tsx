...
import { WorkspaceRenderer } from "@/components/copilot/workspace-renderer";
...
export default function ChatInterface() {
  ...
  const [activeWorkspaceSchema, setActiveWorkspaceSchema] = useState<string | null>(null);
  ...
  // Scan messages to extract active workspace schema
  useEffect(() => {
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (msg.sender === "ai") {
        ...
        if (match && match[0]) {
          setActiveWorkspaceSchema(match[0]);
          break;
        }
      }
    }
  ...
  const streamBotResponse = async (question: string) => {
    setIsTyping(true);
    ...
    try {
      ...
      const isLowConfidence = data.text.includes("sandbox") || data.text.includes("support") || data.text.includes("external accounting");
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          text: data.text,
          timestamp: new Date(),
          confidence: isLowConfidence ? 54 : 95,
      ...
      console.error(err);
      setIsTyping(false);
      toast.error("Failed to connect to AI reasoning service");
      setMessages((prev) => [
        ...prev,
        {
          id: getId(),
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
      ...
      <div className="lg:col-span-7 glass-panel border border-white/5 rounded-2xl h-[72vh] flex flex-col justify-between overflow-hidden shadow-2xl relative">
        {/* Copilot Tier Header */}
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-white flex items-center gap-1.5">
                ...
                {plan === "max" ? (
                  <span className="rounded bg-violet-500/10 text-violet-400 border border-violet-500/20 px-1.5 py-0.5 text-[9px] font-bold uppercase flex items-center gap-0.5 animate-pulse">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-zinc-950/10">
          <AnimatePresence initial={false}>
            {messages.map((msg) => {
              ...
              const cleanedText = isBot ? cleanMessageText(msg.text) : msg.text;
              const displayText = cleanedText || "I have generated the interactive analysis workspace panel.";
      ...
      {/* Right Panel: Interactive Workspace Visualizer */}
      <div className="lg:col-span-5 h-[72vh] overflow-y-auto pr-1 scrollbar-thin">
        <WorkspaceRenderer schema={activeWorkspaceSchema || ""} />
      </div>
    </div>
  );
}
